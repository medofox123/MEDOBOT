...
def shell():
    ...
    printer('_*<مــــعلومات الـفـحـص />*_\n\n', quiet)


    if not args.mini:
        printer('▢ *بـدء الاخـــتباࢪ...*', quiet)
        printer('▢ *جــــاࢪ البــحث عـن الخادم...*', quiet)
        try:
        ...
        if args.server and len(args.server) == 1:
            ...
            printer('▢ *تــــم اختـيار افضل خـــادم...*', quiet)
    ...
    printer('\n▢ *الخـدمه:* %(isp)s' % speedtest.config['client'],
            quiet)
    printer('▢ *شـࢪكه الـنت:* %(sponsor)s\n▢ *المـنطقه التـابعه:* %(name)s [%(d)0.2f km] '
            '\n▢ *مـوقـعك:* %(latency)s ms' % results.server, quiet)
    ...
    if args.download:
        ...
        printer('▢ *⬇️تحـمـيـل البـيانات:* %0.2f M%s/s' %
    ...
    if args.upload:
        speedtest.upload()
        printer('▢ *⬆️ࢪفع البيانات:* %0.2f M%s/s' %
