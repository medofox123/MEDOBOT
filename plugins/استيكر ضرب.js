...
//let { url } = json
let stiker = await sticker(null,s[Math.floor(Math.random() * s.length)], `+${m.sender.split('@')[0]} 𝐹𝛩𝑋 𝑊𝐴 𝐵𝛩𝑇 ${m.mentionedJid.map((user)=>(user === m.sender)? 'alguien ': `+${user.split('@')[0]}`).join(', ')}`)
conn.sendFile(m.chat, stiker, null, { asSticker: true })
} catch (e) { }}
handler.help = ['slap']
handler.tags = ['General']
handler.command = /^slap|ضرب|manotada|abofetear|golpear/i
